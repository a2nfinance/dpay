...
describe('DAO', () => {
  ...
  it('Dao: create a proposal & vote it', async () => {
    const created_proposal = await contract.create_proposal("hello", "hello", 0, [[utils.getDefaultAccounts()[1].address, 1], [utils.getDefaultAccounts()[2].address, 2]], 1, 1, {onAccount: utils.getDefaultAccounts()[1]})
  ...
  it('Dao: fund 1 ae ', async () => {
    ...
    assert.equal(decodedResult.member_fund.get(utils.getDefaultAccounts()[2].address), 1)
  })
